...
def beta_tag(beta):
    return ("{:.3f}".format(float(beta))).rstrip("0").rstrip(".").replace(".", "p")
...
def build_cmd(args, cfg_path, trial, beta):
    return [
        args.python_exe,
        "run.py",
        "-c",
        ...
        str(float(beta)),
        "--mmqs_rho_min",
        str(float(args.mmqs_rho_min)),
        "--mmqs_rho_max",
        str(float(args.mmqs_rho_max)),
        "--prefetch_enabled",
        "--prefetch_top_m",
...
def main():
    ...
    default_run_dir = repo_root / "result" / "experiments" / "ave_results" / "exp3" / "ave_kfit_sensitivity_beta_r850_t6"

    parser = argparse.ArgumentParser(description="AVE dynamic-K fitting beta sensitivity runner (ToT-Lite main method)")
    ...
    parser.add_argument("--betas", type=str, default="1,5,10,50")
    ...
    for beta in betas:
        ...
        for trial in trials:
            ...
            cfg["mmqs"]["beta"] = float(beta)
            cfg["mmqs"]["rho_min"] = float(args.mmqs_rho_min)
            cfg["mmqs"]["rho_max"] = float(args.mmqs_rho_max)

            stem = "ave_kfit_tot_b{}_t{}".format(tag, int(trial))
            ...
            cmd = build_cmd(args, cfg_path, trial, beta)
            print("RUN #{} BETA={} TRIAL={}".format(run_count, beta, trial))
            ...
            summary.append(item)
            print("RUN_DONE BETA={} TRIAL={} EXIT={} ELAPSED={:.2f}s".format(beta, trial, int(proc.returncode), elapsed))
            if proc.returncode != 0:
                if not terminal_only:
                    write_json(run_dir / "run_summary.json", {"runs": summary})
                raise RuntimeError("RUN_FAILED beta={} trial={} err={}".format(
                    beta, trial, "TERMINAL_STDERR" if err_path is None else err_path
                ))
