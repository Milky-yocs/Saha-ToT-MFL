...
def build_cmd(args, cfg_path, trial, w):
    return [
        args.python_exe,
        "run.py",
        "-c",
...
def main():
    ...
    for w in windows:
        for trial in trials:
            ...
            cmd = build_cmd(args, cfg_path, trial, w)
            print("RUN #{} W={} TRIAL={}".format(run_count, w, trial))
            ...
            summary.append(item)
            print("RUN_DONE W={} TRIAL={} EXIT={} ELAPSED={:.2f}s".format(w, trial, int(proc.returncode), elapsed))
            if proc.returncode != 0:
                if not terminal_only:
                    write_json(run_dir / "run_summary.json", {"runs": summary})
                raise RuntimeError("RUN_FAILED w={} trial={} err={}".format(
                    w, trial, "TERMINAL_STDERR" if err_path is None else err_path
                ))
